...
| S3-b | ✅ **[해결됨] 설치·import 개수 문제** | 컴포넌트 6개를 각각 설치하고 스타일시트 4개를 import해야 하던 온보딩 마찰 → **`softium-ui` 통합 패키지** 추가(`npm i softium-ui` + `import 'softium-ui/styles.css'` 한 줄). 개별 패키지는 그대로 유지되어 필요한 것만 설치하는 선택지도 남음. 구현 노트: `Header`가 ui(앱셸)/table-react(테이블)에 모두 있어 `export *`로는 ESM에서 조용히 누락되고 TS는 에러 → 앱셸 쪽을 `AppHeader`로 재export하고 `softium-ui/ui` 서브패스로 원본 이름 보존. CSS는 토큰 레이어를 1회만 인라인해 개별 합산 대비 ~18KB 절감. |
...
- [x] R-4 🟡 GitHub Actions CI — **완료**. `ci.yml`(lint→build→typecheck→test + 발행물 가드 2종: flat CSS에 `@import` 0개, React 번들 `"use client"` 유지) + `release.yml`(changesets action). 착수 시 발견·수정: ①lint 16 errors(포매팅/임포트 정렬 자동수정 + hook deps는 사유 명시한 **한 줄** biome-ignore — 여러 줄로 쪼개면 억제가 안 됨) ②CI에서 typecheck가 build보다 먼저면 workspace 의존 `.d.ts` 미존재로 TS2307 → **build 선행** 필수 ③`changeset version`이 package.json을 재포맷해 포매터 검사와 충돌 → `version` 스크립트에 `pnpm format` 체이닝 ④Actions의 PR 생성 권한(`can_approve_pull_request_reviews`) 필요.
